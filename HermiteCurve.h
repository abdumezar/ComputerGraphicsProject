...
void DrawCardinalSpline(HDC hdc,Vector2 P[],int n, double c ,COLORREF s) {
    double c1 = 1 - c;
    Vector2 T0(c1 * (P[2].x - P[0].x), c1 * (P[2].y - P[0].y));
    for (int i = 2; i < n - 1; i++) {
        Vector2 T1(c1 * (P[i + 1].x - P[i - 1].x), c1 * (P[i + 1].y - P[i - 1].y));
        Vector2 L[4] = {P[i-2],T0,T1,P[i-1]};
        HermiteCubicCurve::DrawHermite(hdc,HermiteCubicCurve(L), s);
        //DrawHermite(hdc, L, s);
        T0 = T1;
        }
}
